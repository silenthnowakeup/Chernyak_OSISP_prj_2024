...
void assistant::load_settings()
{
	...
	if (file.good())
	{
		...
		while (!file.eof())
		{
			file >> u_name >> s_speed >> s_volume >> s_pitch >> t_speed >> voice;
		}
...
void assistant::save_settings(string un = "silenth", int ss = 160, int sa = 100, int sp = 40, int ts = 40, string v = "english-us")
{
	ofstream file;
	file.open(".user-settings", ios::out);

	file << un << " " << ss << " "  << sa << " "  << sp << " "  << ts << " " << v;
